...
This suggests that, **if we are willing to let the ANOVA test decide our best model** than that would be the model produced by forward selection, with predictors `lcavol`, `lweight` and `svi_f`. But...
...
So we'll need some additional methods for selecting variables, and for then validating the results once we complete the selection process. 
