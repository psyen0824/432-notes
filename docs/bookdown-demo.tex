...
\setlength{\droptitle}{-2em}
  ...
  \postdate{\par}
  \date{Version: 2018-01-16 21:03:39}
...
\section{A Small Study}\label{a-small-study}
...
\tightlist
\item
  The outcome of interest is \texttt{physhealth}.
\item
  ...
  \item
    \texttt{bmi} = body mass index (treated as qualitative and
    continuous)
  \end{itemize}
\end{itemize}

\subsection{Some exploratory data
analysis}\label{some-exploratory-data-analysis}

Counting things can be amazingly useful.

\subsubsection{How many respondents had exercised in the past 30
days?}\label{how-many-respondents-had-exercised-in-the-past-30-days}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{smartcle2 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(exerany)}
\end{Highlighting}
\end{Shaded}
...
This counting approach works for quantitative variables with discrete
sets of possible values, like \texttt{physhealth}, which must be an
integer between 0 and 30.

\subsubsection{\texorpdfstring{What's the distribution of
\texttt{physhealth}?}{What's the distribution of physhealth?}}\label{whats-the-distribution-of-physhealth}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{smartcle2 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(physhealth)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{smartcle2 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(bmi }\OperatorTok{<}\StringTok{ }\DecValTok{30}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{smartcle2 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(bmi }\OperatorTok{<}\StringTok{ }\DecValTok{30}\NormalTok{, exerany)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{\texorpdfstring{Comparing \texttt{physhealth} summaries
by obesity
status}{Comparing physhealth summaries by obesity status}}\label{comparing-physhealth-summaries-by-obesity-status}

Can we compare the \texttt{physhealth} means, medians and
...
\StringTok{    }\KeywordTok{summarize}\NormalTok{(}\KeywordTok{mean}\NormalTok{(physhealth), }\DataTypeTok{median =} \KeywordTok{median}\NormalTok{(physhealth), }
              \DataTypeTok{q75 =} \KeywordTok{quantile}\NormalTok{(physhealth, }\FloatTok{0.75}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{    }\KeywordTok{skim}\NormalTok{(bmi, physhealth)}
\end{Highlighting}
\end{Shaded}
...
We'll build an exploratory figure (or several) to show the relationship
between \texttt{bmi} and \texttt{physhealth} within each of the two
\texttt{exerany} groups.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggplot}\NormalTok{(smartcle2, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ bmi, }\DataTypeTok{y =}\NormalTok{ physhealth, }
                      \DataTypeTok{group =}\NormalTok{ exerany, }\DataTypeTok{color =} \KeywordTok{factor}\NormalTok{(exerany))) }\OperatorTok{+}
